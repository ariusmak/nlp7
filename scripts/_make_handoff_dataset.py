...
linked = pd.read_parquet(ROOT / "data/interim/posts_linked_to_events.parquet")
posts_slim = linked[[
    "id", "ticker", "event_id", "rdq", "window_start", "post_dt",
    "title", "selftext", "has_real_body",
]].copy()
posts_out = HANDOFF / "posts_linked_to_events_slim.parquet"
posts_slim.to_parquet(posts_out, index=False, compression="zstd")

evs = pd.read_parquet(ROOT / "data/processed/event_modeling_dataset.parquet")
skeleton_cols = [
    "ticker", "event_id", "rdq", "calendar_year",
    "earnings_timing", "timing_used",
    "ret5_pre", "ret20_pre", "vol20_pre", "abvol_pre",
    "target_ret3", "target_start_date", "target_end_date",
]
skeleton = evs[skeleton_cols].copy()
skel_out = HANDOFF / "event_modeling_skeleton.parquet"
skeleton.to_parquet(skel_out, index=False, compression="zstd")
...
import os
print(f"wrote: {posts_out}  ({os.path.getsize(posts_out)/1e6:.2f} MB)")
print(f"wrote: {skel_out}   ({os.path.getsize(skel_out)/1e6:.3f} MB)")
